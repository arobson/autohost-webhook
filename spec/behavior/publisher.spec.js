...
describe( "Publisher", function() {
	...
	describe( "when receiving an event with no registered hooks", function() {
		...
		describe( "with a hook that excludes the event", function() {
			var addedMock;
			before( function() {
				addedMock = sinon.mock( hooks );
				host1 = nock( "http://hook.host.one.net", {
				...
				.post( "/api/callme",
				{
					test: true,
					topic: "good.thing",
					message: "good thing that happened. yay."
				} )
				.reply( 200 );
			...
			it( "should publish event to 2 subscribed and available hooks", function() {
				host1.done();
				host2.done();
			} );
		} );
